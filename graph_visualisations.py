...
cleaned_data.groupby('native_status_graph').size().plot(kind='pie', autopct=label_function, textprops={'fontsize': 10},
                                 colors=['violet', 'red'], ax=axes[1,1])
axes[0,1].set_ylabel('Race', size=22)
axes[1,1].set_ylabel('US Native Country', size=22)
...
fig12, axes12 = plt.subplots(2, 2,figsize=(10, 8))
ax7 = sns.countplot(ax = axes12[0,0], x = 'occupation', hue = 'income_over_50_graph', data = cleaned_data).set(
    xlabel='Occupation')
axes12[0,0].set_xticklabels(axes12[0,0].get_xticklabels(), rotation=90, ha="right")
axes12[0,0].legend(loc='best')

ax8 = sns.violinplot(ax = axes12[0,1], y='income_over_50_graph', x = 'age', hue="income_over_50_graph", data = cleaned_data, legend_out = False).set(ylabel='Income over 50')
axes12[0,1].legend(loc='best')
plt.tight_layout()
plt.show()
